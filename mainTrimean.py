...
def main(dataset, algorithm,batch_size, learning_rate,num_glob_iters,
    ...
    for i in range(times):
        ...
        if(algorithm == "Trimean"):
            server = Trimean(device, dataset, algorithm, model, batch_size, learning_rate,num_glob_iters, local_epochs,n_attackers,attacker_type, iid,int(numusers*select_users_ratio),datadir,beta,partition,numusers,localstep)
        server.train()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--attacker_type", type=str, default="Fang", choices=["LIE","Fang","NDSS","Mimic","MinMax","MinSum","LabelFlip","DynamicLabelFlip"])
    parser.add_argument("--n_attackers", type=int, default=5, help="The number of attackers")
