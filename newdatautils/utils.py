...
# from model import *
from newdatautils.datasets import MNIST_truncated, CIFAR10_truncated, CIFAR100_truncated, ImageFolder_custom, SVHN_custom, \
    FashionMNIST_truncated, CustomTensorDataset, CelebA_custom, FEMNIST, Generated, genData, STL10_truncated
...
root_path = 'dataset/betelgeuse/datautils/'
cifar10_path = root_path + 'image_match_client_index.txt'
stl_path = root_path + 'image_match_client_index_stl.txt'
mnist_path = root_path + 'image_match_client_index_mnist.txt'
svhn_path="dataset/svhn_data"
...
def load_mnist_data(datadir):
    ...
    file_path = mnist_path
    # X_train = X_train.data.numpy()
    # y_train = y_train.data.numpy()
    # X_test = X_test.data.numpy()
    # y_test = y_test.data.numpy()

    Z_train = add_label(X_train, file_path)
    #print(Z_train)
    #print("Finish add z_train to data")
    # y_train = y_train.numpy()
    # y_test = y_test.numpy()
    return X_train, y_train, X_test, y_test, Z_train
...
def load_stl10_data(datadir):
    stl10_untrain_data = STL10_truncated(datadir, unlabel=True, train=True, download=True)
    X_train = stl10_untrain_data.data

    file_path = 'dataset/betelgeuse/config/image_match_client_index_stl.txt'
    z_train = add_label(X_train, file_path)
    print(z_train)
    print("Finish add z_train to data")
...
def partition_data(dataset, datadir, logdir, partition, n_parties, beta=0.4):
    ...
    if dataset == 'mnist':
        X_train, y_train, X_test, y_test, z_train = load_mnist_data(datadir)
    elif dataset == 'fmnist':
        X_train, y_train, X_test, y_test = load_fmnist_data(datadir)
    elif dataset == 'cifar10':
        X_train, y_train, X_test, y_test= load_cifar10_data(datadir)
    elif dataset == 'stl10':
        X_train, z_train = load_stl10_data(datadir)
    elif dataset == 'svhn':
        X_train, y_train, X_test, y_test = load_svhn_data(svhn_path)
    elif dataset == 'celeba':
    ...
    elif partition == "noniid-featuredir":
        min_size = 0
        min_require_size = 3
        # min_require_size represnets the required minimal datasize of a single clinet.
        # K = 10
        K = 7
        if dataset == 'cifar10':
            K = 7
    ...
    #traindata_cls_counts = record_net_data_stats(z_train, net_dataidx_map, logdir)
...
def get_dataloader(dataset, datadir, dataidxs=None):
    if dataset in (
        ...
        if dataset == "tinyimagenet":
            ...
            # server_test_ds = dl_obj(datadir, train=False, transform=transform_test, download=True)
            # server_train_ds = dl_obj(datadir, train=True, transform=transform_train, download=True)

        # train_dl = data.DataLoader(dataset=train_ds, batch_size=train_bs, shuffle=True, drop_last=False)
        # server_test_dl = data.DataLoader(dataset=server_test_ds, batch_size=100, shuffle=False, drop_last=False)
        # server_train_dl = data.DataLoader(dataset=server_train_ds, batch_size=100, shuffle=False, drop_last=False)

    # return train_dl, test_dl, train_ds, test_ds
    # return train_ds, server_test_dl,server_train_dl
    return train_ds
