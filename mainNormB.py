...
from Server.ServerNormB import NormBound
...
def main(dataset, algorithm,batch_size, learning_rate,num_glob_iters,
    ...
    for i in range(times):
        ...
        # select algorithm
        if(algorithm == "NormBound"):
            server = NormBound(device, dataset, algorithm, model, batch_size, learning_rate,num_glob_iters, local_epochs,n_attackers,attacker_type, iid,int(numusers*select_users_ratio),datadir,beta,partition,numusers,localstep)
        server.train()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--algorithm", type=str, default="NormBound")
