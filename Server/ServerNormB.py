...
class NormBound(Server):
    ...
    def aggregate_grads_NormB(self):

        user_param = []
        for i in range(len(self.selected_users)):
            param = self.selected_users[i].get_params()
            ...
            user_param = param[None, :] if len(user_param) == 0 else torch.cat((user_param, param[None, :]),
                                                                               0)
        user_norm=torch.norm(user_param, dim=1).cpu()
        norm_bound=torch.median(user_norm).cpu()
        norm_bound=norm_bound*2
        print(user_norm)
        print(norm_bound)
        #user_norm=user_norm.cuda()
        #norm_bound=norm_bound.cuda()
        global_para=self.model.state_dict()

        idx = 0
        for i in range(len(self.selected_users)):
            net_para = self.selected_users[i].model.cpu().state_dict()
            reweight_ratio=min(norm_bound/user_norm[i],1)
            print(reweight_ratio)
            #reweight_ratio=1
            if idx == 0:
                for key in net_para:
                    global_para[key] = net_para[key] / len(self.selected_users)*reweight_ratio
            else:
                for key in net_para:
                    global_para[key] += net_para[key] / len(self.selected_users)*reweight_ratio
            idx = idx + 1
    ...
    def aggregate_grads_NormB_new(self):
        user_updates = []
        ...
        for user in self.selected_users:
            ...
            user_updates = param[None, :] if len(user_updates) == 0 else torch.cat((user_updates, param[None, :]),
                                                                                   0)
        user_norm = torch.norm(user_updates, dim=1).cpu()
        norm_bound = torch.median(user_norm).cpu()*1.7
        print(user_norm)
        print(norm_bound)
        for i in range(len(user_updates)):
            reweight_ratio=min(norm_bound/user_norm[i],1)
            user_updates[i]=user_updates[i]*reweight_ratio
        ...
        for glob_iter in range(self.num_glob_iters):
            ...
            for user in self.selected_users:
                if user in self.attackers:
                    user.load_malicious_state(malicious_state)
            '''
            self.aggregate_grads_NormB_new()
